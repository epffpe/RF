...
%
 ...
 function scScales(a) 
 %argument a added by ICS. 9-1-02
 % if called with argument only draws r scale
 
 % DRAW BASE LINES
 hold on;
 if nargin==1
    yB = -1.40; %custom values
    xB = -1.0;
 else
    yB = 1.5; %Default values
    xB = 0.05;
 end
 ...
 if nargin==1  %added by ICS. 9-1-02
    h = text(xB+0.5, yB+0.18, 'Reflection Coefficient |\Gamma|');
